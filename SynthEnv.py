...
class SynthEnv(py_environment.PyEnvironment):
    ...
    def __init__(self, N_synth_params, 
        ...
        print(lookup_table_path.split('/')[1])
        ...
        self.features_keep = features_keep # features to use for training
        self.N_features = len(self.features_keep) # number of features used for training
        self.lookup_table_path = lookup_table_path # path to the csv lookup table for the synthesizer 
        self.target_corpus_path = target_corpus_path # path to target corpus feature csv files folder
        ...
        param_names = [f'p{num_param}' for num_param in range(self.N_synth_params)]
        features_keep_timbre = param_names + self.features_keep
        lookup_table = pd.read_csv(lookup_table_path)[features_keep_timbre].values
    ...
    def _step(self, action):
        ...
        if self.live:
            ...
            self.osc_client_optimal.send_message("/result", msg)
    ...
    def sendToSynth(self, synthparams):
        ...
        msg = ' '.join(resultArray)
        self.osc_client.send_message("/result", msg)
...
def test_metrics(model_dir, test_steps):
    ...
    py_environment = StaticSynthMatchingEnv(N_synth_params, 
                                             features_keep, 
                                             lookup_table_path, 
                                             target_corpus_path,
                                             reward_weights, 
    ...
    for step in range(test_steps):
        ...
        avg_RMSE = 0.5 - cube(avg_reward/4)
